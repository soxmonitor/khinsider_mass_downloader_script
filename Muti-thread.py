...
# 下载音轨
def download_tracks(album_url, download_flac, download_wav, download_mp3, max_threads=5):
    ...
    for link in track_links:
        ...
        if not os.path.exists(safe_file_name):
            print(f"Missing file: {safe_file_name}. Retrying download...")
    ...
    for link in soup.find_all('a'):
        if 'href' in link.attrs:
            ...
            if full_link.endswith(('.mp3', '.flac', '.wav')):
                track_links.add(full_link)  # 使用add而不是append，保证链接唯一
